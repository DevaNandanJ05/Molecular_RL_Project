...
def _load_local_smiles(path: str) -> list:
    """Load SMILES from a local file, or download the TDC DRD2 dataset as a fallback."""
    if not os.path.exists(path):
        print(f"\n[WARNING] No local SMILES file found at {path}")
        print("[WARNING] Since the ChEMBL API is down, automatically switching to the Therapeutics Data Commons (TDC) dataset...")
        print("          Downloading DRD2 actives from Harvard Dataverse...")
        
        import urllib.request
        import csv
        
        # TDC DRD2 Dataset (PairMolGen) hosted on Harvard Dataverse
        tdc_url = "https://dataverse.harvard.edu/api/access/datafile/4170957"
        ...
        try:
            ...
            with open(temp_tsv, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    if len(row) >= 2:
                        unique_smiles.add(row[0].strip())
                        unique_smiles.add(row[1].strip())
            
            # Write out the curated SMILES
            ...
            print(f"[SFT] Successfully extracted {len(unique_smiles)} DRD2 SMILES from TDC dataset.")
            # Clean up temp file
            if os.path.exists(temp_tsv):
                os.remove(temp_tsv)
                
        except Exception as e:
            print(f"[ERROR] Failed to download TDC fallback dataset: {e}")
            print("        Generating 20-molecule emergency dataset...")
            # A curated list of known, high-affinity DRD2 ligands
            fallback_smiles = [
            ...
            with open(path, 'w') as f:
                for smi in fallback_smiles:
                    f.write(smi + '\n')
            print(f"[SFT] Wrote {len(fallback_smiles)} emergency fallback SMILES to {path}")
