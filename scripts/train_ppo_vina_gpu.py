...
def get_vina_gpu_config() -> dict:
    ...
    return {
        ...
        "ifars_enabled": True,
        "ifars_reference_json": os.path.join(project_root, "data", "reference", "haloperidol_fingerprint.json"),
        "ifars_gate_threshold": -6.0,  # Run PLIP only when Vina docking score <= -6.0 kcal/mol
        "ifars_w_overlap": 2.0,        # Weight for fraction of Haloperidol's 18 interaction keys matched
        "ifars_w_critical": 3.0,       # Weight for fraction of 12 critical residues matched
        "ifars_w_asp114": 2.0,         # Dedicated bonus for engaging essential Asp114 anchor
...
class RewardOracleVinaGPU:
    def __init__(
        ...
        ifars_enabled=True,          # Enable IF-ARS interaction fingerprint reward shaping
        ifars_reference_json=None,   # Path to haloperidol_fingerprint.json
        ifars_gate_threshold=-6.0,   # Run PLIP only when docking score <= threshold
        ifars_w_overlap=2.0,         # Weight for overall fingerprint overlap
        ifars_w_critical=3.0,        # Weight for critical residue overlap
        ifars_w_asp114=2.0,          # Weight for essential Asp114 anchor bonus
    ):
        ...
        # IF-ARS (Interaction Fingerprint-guided Adaptive Reward Shaping)
        self.ifars_enabled = ifars_enabled
        self.ifars_gate_threshold = float(ifars_gate_threshold)
        self.ifars_w_overlap = float(ifars_w_overlap)
        self.ifars_w_critical = float(ifars_w_critical)
        self.ifars_w_asp114 = float(ifars_w_asp114)

        self.plip_analyzer = None
        if self.ifars_enabled:
            ref_json = ifars_reference_json or os.path.join(project_root, "data", "reference", "haloperidol_fingerprint.json")
            try:
                ...
                self._logger.warning(f"[RewardOracle] Failed to initialize PLIPAnalyzer: {e}. IF-ARS disabled.")
                self.plip_analyzer = None
            ...
            try:
                ...
                if cached is not None:
                    if isinstance(cached, dict):
                        docking_score = cached.get("score")
                        ifars_overlap = cached.get("ifars_overlap", 0.0)
                        critical_overlap = cached.get("critical_overlap", 0.0)
                        has_asp114 = cached.get("has_asp114", False)
                        ifars_bonus = cached.get("ifars_bonus", 0.0)
                    else:
                        docking_score = float(cached)
                    cache_hit = True
            except Exception:
                pass  # IPC failure — fall through to actual docking

        if not cache_hit:
            docking_score, docked_out_path = self._run_vina_gpu_docking(mol)

            # IF-ARS: If docking affinity is within the binding pocket threshold (<= -6.0 kcal/mol),
            # analyze the 3D binding mode with PLIP against Haloperidol's reference fingerprint.
            if (self.ifars_enabled and self.plip_analyzer is not None and 
                docking_score is not None and docking_score <= self.ifars_gate_threshold and 
                docked_out_path is not None and os.path.exists(docked_out_path)):
                try:
                    plip_res = self.plip_analyzer.score_docked_pose(docked_out_path)
                    ifars_overlap = plip_res.get("overlap_score", 0.0)
                    critical_overlap = plip_res.get("critical_overlap", 0.0)
                    matched_keys = plip_res.get("matched_keys", [])
                    has_asp114 = any("ASP114" in k for k in matched_keys)
                    ifars_bonus = (self.ifars_w_overlap * ifars_overlap) + \
                                  (self.ifars_w_critical * critical_overlap) + \
                                  (self.ifars_w_asp114 if has_asp114 else 0.0)
                except Exception as e:
                    self._logger.warning(f"[IF-ARS] PLIP scoring failed: {e}")
                finally:
                    if os.path.exists(docked_out_path):
                        try: os.remove(docked_out_path)
                        except OSError: pass
            elif docked_out_path is not None and os.path.exists(docked_out_path):
                try: os.remove(docked_out_path)
                except OSError: pass
            ...
            if self.docking_cache is not None:
                try:
                    self.docking_cache.put(canonical, {
                        "score": docking_score,
                        "ifars_overlap": ifars_overlap,
                        "critical_overlap": critical_overlap,
                        "has_asp114": has_asp114,
                        "ifars_bonus": ifars_bonus,
                    })
                except Exception:
                    pass

        if docking_score is not None and docking_score < 0:
            # Combined reward: Inverted Docking affinity + QED + IF-ARS potential - Structural penalties
            total_reward = abs(docking_score) + (qed_score * 1.5) + ifars_bonus - penalty
    ...
    def _run_vina_gpu_docking(self, mol) -> float:
        ...
        temp_docked_out = os.path.join(worker_temp_dir, f"docked_out_{thread_id}.pdbqt")
        config_file = os.path.join(worker_temp_dir, f"config_{thread_id}.txt")
        
        try:
            mol_3d = Chem.AddHs(mol)
            if AllChem.EmbedMolecule(mol_3d, AllChem.ETKDG()) != 0:
                return None, None   # Distinct from 0.0 — signals "undockable" to caller
            ...
            try:
                ...
                if os.path.exists(temp_docked_out):
                    try: os.remove(temp_docked_out)
                    except OSError: pass
                return None, None
            ...
            if match:
                ...
                return score, (temp_docked_out if os.path.exists(temp_docked_out) else None)
...
class MolGenEnvVinaGPU(gym.Env):
    def __init__(
        ...
        ifars_enabled=True,
        ifars_reference_json=None,
        ifars_gate_threshold=-6.0,
        ifars_w_overlap=2.0,
        ...
        self.oracle = RewardOracleVinaGPU(
            ...
            ifars_enabled=ifars_enabled,
            ifars_reference_json=ifars_reference_json or VINA_GPU_CONFIG.get("ifars_reference_json"),
            ifars_gate_threshold=ifars_gate_threshold,
            ifars_w_overlap=ifars_w_overlap,
            ifars_w_critical=ifars_w_critical,
            ifars_w_asp114=ifars_w_asp114,
        )
    ...
    def step(self, action):
        ...
        if is_drug_sized or force_end:
            ...
            if mol is not None:
                ...
                # IF-ARS Interaction Metrics
                info["ifars_overlap"] = res.get("ifars_overlap", 0.0)
                info["critical_overlap"] = res.get("critical_overlap", 0.0)
                info["has_asp114"] = res.get("has_asp114", False)
                info["ifars_bonus"] = res.get("ifars_bonus", 0.0)
                ...
                info["curriculum_phase"] = "docking" if use_docking else "warmup"
                info["episode_length"] = self.step_idx
                info["ifars_overlap"] = 0.0
                info["critical_overlap"] = 0.0
                info["has_asp114"] = False
                info["ifars_bonus"] = 0.0
...
class MoleculeLoggingCallback(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        with open(self.csv_path, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "ifars_overlap", "critical_overlap", "asp114_hit", "ifars_bonus",
            ])
    ...
    def _on_step(self) -> bool:
        for idx, info in enumerate(self.locals.get("infos", [])):
            ...
            overlap = info.get("ifars_overlap", 0.0)
            if overlap > self.best_ifars_overlap:
                self.best_ifars_overlap = overlap
            ...
            with open(self.csv_path, "a", newline="") as f:
                ...
                writer.writerow([
                    ...
                    f"{overlap:.4f}",
                    f"{info.get('critical_overlap', 0.0):.4f}",
                    info.get("has_asp114", False),
                    f"{info.get('ifars_bonus', 0.0):.3f}",
                ])
            ...
            # IF-ARS Metrics
            self.logger.record("ifars/overlap_score", overlap)
            self.logger.record("ifars/critical_overlap", info.get("critical_overlap", 0.0))
            self.logger.record("ifars/asp114_hit", 1.0 if info.get("has_asp114", False) else 0.0)
            self.logger.record("ifars/bonus", info.get("ifars_bonus", 0.0))
            ...
            if self.total_episodes % self.summary_freq == 0:
                ...
                phase_str = "[IF-ARS DOCKING]" if phase == "docking" else "[WARMUP (QED-only)]"
                top_cand = self.best_smiles if self.best_smiles else (self.best_reward_smiles or "N/A")
                crit_overlap_cur = info.get("critical_overlap", 0.0)
                asp_hit_cur = "YES" if info.get("has_asp114", False) else "NO"
                print(
                    f"\n{'='*70}\n"
                    ...
                    f"  IF-ARS: Overlap={overlap:.1%} (Best: {self.best_ifars_overlap:.1%})  |  "
                    f"Critical={crit_overlap_cur:.1%}  |  Asp114 Anchor={asp_hit_cur}\n"
...
def train(
    ...
    print(f"  IF-ARS Pipeline : {'ENABLED (PLIP + Haloperidol Reference)' if config.get('ifars_enabled', True) else 'Disabled'}")
    print(f"  IF-ARS Gate     : Affinity <= {config.get('ifars_gate_threshold', -6.0)} kcal/mol")
    print("=" * 70)
    ...
    env = make_vec_env(
        ...
        env_kwargs={
            ...
            "ifars_enabled": config.get("ifars_enabled", True),
            "ifars_reference_json": config.get("ifars_reference_json"),
            "ifars_gate_threshold": config.get("ifars_gate_threshold", -6.0),
            "ifars_w_overlap": config.get("ifars_w_overlap", 2.0),
            "ifars_w_critical": config.get("ifars_w_critical", 3.0),
            "ifars_w_asp114": config.get("ifars_w_asp114", 2.0),
        },
